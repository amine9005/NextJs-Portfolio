      ...
      <div className="max-w-6xl w-full px-6">
        <div className="flex flex-col md:flex-row gap-16 mt-4">
          <div className="flex  w-full gap-3">
            <div className="flex max-h-100 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden overflow-y-scroll flex-col gap-3">
              {product.images.map((image, index) => (
                <div
                  key={index}
                  onClick={() => setThumbnail(image)}
                  className="border min-h-24 max-w-24 border-gray-500/30 rounded overflow-hidden cursor-pointer"
                >
                  <Image
                    src={image}
